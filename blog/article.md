...
작업이 길어지는 경우에는 일감 처리 콘솔에서 이어갑니다. 콘솔은 `AI 분석`, `repo 확정`, `workspace 준비`, `업무 처리`, `테스트`, `보고/완료`를 단계로 보여주고, 각 단계마다 승인 버튼과 다음 행동을 제공합니다. `AI 작업 지휘관`은 현재 상태를 기준으로 작업 계획, 브랜치 전략, 테스트 추천, 컨벤션 점검, 보고 초안을 만들어 중간중간 판단을 돕습니다.
