...
| 프로젝트/일감 | 수동 관리 프로젝트와 Jira 연결 프로젝트, 프로젝트별 일감 부여 | 여러 프로젝트의 작업을 같은 workflow 기준으로 분리 |
...
최근에는 이 흐름을 `프로젝트 -> 일감 -> workflow` 구조로 좁혔습니다. 프로젝트는 `수동 관리` 또는 `Jira 연결` 방식으로 만들 수 있고, 각 프로젝트 카드에서 수동 일감을 부여하거나 Jira 일감을 가져와 같은 처리 workflow로 흘려보냅니다. 앱은 각 단계에서 다음 행동과 리스크를 보여주고, `AI 작업 지휘관` 패널은 현재 상태를 바탕으로 분석, 작업 계획, 브랜치 전략, 테스트 추천, 보고 초안을 정리합니다.
...
|---|---|---|
| 프로젝트 선택 | 수동 관리 프로젝트와 Jira 연결 프로젝트를 나누고 프로젝트별 일감 수를 보여줍니다. | 여러 프로젝트의 업무가 섞이지 않게 시작점을 고릅니다. |
| 일감 부여 | 프로젝트 카드에서 수동 일감을 등록하거나 Jira 일감을 가져옵니다. | 일감이 생성되는 순간부터 어느 프로젝트의 workflow인지 분명해집니다. |
...
프로젝트를 만들면 DevPilot는 그 아래에 일감을 부여하고 처리할 수 있게 합니다. 프로젝트는 Jira를 쓰지 않는 수동 관리 프로젝트와 Jira 프로젝트 키를 가진 연결 프로젝트로 나뉩니다.

- 수동 관리 프로젝트: 직접 일감을 등록하고 Git/workspace 중심으로 처리
- Jira 연결 프로젝트: Jira 프로젝트 키를 기준으로 일감을 가져오고 로컬 workflow로 처리
- 프로젝트 카드: 진행 중/승인 대기/전체 일감을 확인하고 일감 등록 또는 Jira 가져오기를 실행
...
- [화면 기반 사용자 매뉴얼](docs/USER_MANUAL.md)
- [일감 처리 워크플로우](docs/ISSUE_WORKFLOW.md)
...
-> S3 portfolio-feed/devpilot/
```

## 현재 상태

DevPilot는 현재 **개인 실사용 베타**에 가깝습니다. 매일 개발 업무 옆에 켜두고 사용할 수 있는 수준까지 왔고, 화면 기반 사용자 매뉴얼과 포트폴리오 게시 패키지도 함께 정리했습니다.

다음 개선 후보는 다음과 같습니다.

- 프로젝트별 대시보드에서 승인 대기, 테스트 필요, 보고 필요 상태를 더 선명하게 표시
- AI 작업 지휘관 결과를 repository 선택, 브랜치 전략, 테스트 명령 추천 버튼과 더 직접 연결
- Jira 비활성 환경에서 Jira 문구와 경고를 더 줄이고 수동 workflow 중심 문구 강화
- workflow 단계별 승인/완료 버튼의 UX와 상태 전환 규칙 통일
- 실패 상황별 복구 가이드와 workspace cleanup/rebase 충돌 처리 안내
- 큰 SwiftUI 화면의 추가 분리와 화면 단위 테스트 보강
