...
from devpilot.integrations.git_repos import ahead_behind, configured_repositories, current_branch, fetch, pull_ff_only, status_porcelain
...
def _format_blocks(
    ...
    blocks = [
        header_block("DevPilot 출근 Git 정비 결과"),
        fields_block(
            [
                f"*자동 최신화*\n{len(updated)}개",
                f"*확인 필요*\n{len(attention)}개",
