...
from devpilot.http import json_request
from devpilot.integrations.git_repos import configured_repositories
from devpilot.integrations.jira import JiraClient
from devpilot.integrations.slack import SlackClient, header_block, list_channels, section_block
...
def _send_alert(config: AppConfig, checks: list[HealthCheck]) -> None:
    ...
    SlackClient(config.slack, destination="alerts").send(
        "DevPilot API 헬스체크 경고",
        blocks=[
            header_block("DevPilot API 헬스체크 경고"),
            section_block(text),
        ],
    )
