...
def _slack_checks(config: AppConfig) -> list[str]:
    if not config.features.notifications:
        return [_check("slack.notifications", True, "선택 기능 비활성화")]
    checks = [
    ...
    if config.features.jira:
        checks.insert(3, _secret_check("slack.jira_daily", config.slack.destination_configured("jira_daily"), "Jira 브리핑 채널"))
    return checks
