...
from devpilot.features.assignees import resolve_assignee
from devpilot.features.issue_repositories import IssueRepositoryLink, issue_repository_links
from devpilot.integrations.git_repos import configured_repositories, git
from devpilot.integrations.jira import JiraClient
from devpilot.integrations.slack import (
    SlackClient,
    actions_block,
    button_element,
    context_block,
...
def _format_issue_actions_markdown(issue: dict[str, Any], repo_links: dict[str, IssueRepositoryLink]) -> str:
    ...
    links = [
        _slack_link(
            f"devpilot://jira/link?{urlencode({'issue': issue_key, 'summary': summary})}",
            "레포 연결 선택",
        )
    ]
    ...
    if repo_link:
        ...
        links.append(
            _slack_link(
                f"devpilot://branch/create?{query}",
