...
from devpilot.features.dev_assistant import evening_check, morning_briefing
from devpilot.features.jira_daily import format_today_items
...
def tick(config: AppConfig, *, task_name: str | None = None, dry_run: bool = False) -> str:
    ...
    state = read_state()
    lines = [f"DevPilot automation tick - {now.isoformat(timespec='seconds')}"]
...
def _send_failure_alert(config: AppConfig, task: str, exc: Exception) -> None:
    try:
        SlackClient(config.slack, destination="alerts").send(
            f"DevPilot 자동 실행 실패: {task}",
            blocks=[section_block(f"*DevPilot 자동 실행 실패*\n작업: `{task}`\n오류: `{exc}`")],
        )
    except Exception:
        return
