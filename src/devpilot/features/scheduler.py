...
from devpilot.app_state import default_config_path
from devpilot.config import AppConfig, ScheduleConfig
from devpilot.metadata import CLI_MODULE, LAUNCHD_LABEL_PREFIX
...
def install_schedules(config: AppConfig) -> str:
    _require_macos()
    lines = [f"DevPilot 스케줄 설치 - {SYSTEM_LABEL}"]
...
def uninstall_schedules() -> str:
    _require_macos()
    lines = [f"DevPilot 스케줄 제거 - {SYSTEM_LABEL}"]
    for task, label in TASK_LABELS.items():
        _uninstall_launchd(task)
...
def schedule_status(config: AppConfig) -> str:
    _require_macos()
    lines = [f"DevPilot 스케줄 설정 상태 - {SYSTEM_LABEL}"]
    for task, label in TASK_LABELS.items():
        schedule = config.schedules[task]
...
def _require_macos() -> None:
    ...
    if system != SUPPORTED_SYSTEM:
        raise RuntimeError(f"지원하지 않는 OS입니다: {system}. DevPilot는 현재 macOS 전용입니다.")
...
def _install_launchd(task: str, schedule: ScheduleConfig) -> None:
    ...
    program_arguments = "\n".join(f"    <string>{_xml_escape(arg)}</string>" for arg in _devpilot_tick_command(task))
    plist.write_text(
...
def _launchd_label(task: str) -> str:
    return f"{LAUNCHD_LABEL_PREFIX}.{task.replace('_', '-')}"
...
def _devpilot_tick_command(task: str) -> list[str]:
